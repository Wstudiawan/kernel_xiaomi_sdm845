...
export KBUILD_COMPILER_STRING="$(${KERNEL_DIR}/gcc/bin/gcc --version | head -n 1 | perl -pe 's/\(http.*?\)//gs' | sed -e 's/  */ /g')"
export ARCH=arm64
...
function zipping() {
    cd AnyKernel || exit 1
    zip -r9 perf_but_not_stock Kernel-${TANGGAL}.zip *
    cd ..
}
sticker
sendinfo
